...
window.headshot = {
    ...
    if (message.url) { 
      var image_url =  message.url;
      var first_photo_div = $("#account_live_photo_1");
      var second_photo_div = $("#account_live_photo_2");
      // console.log(array_divs_values);                 

      first_photo_div.val() ? second_photo_div.val(image_url) : first_photo_div.val(image_url)

      var array_divs = [first_photo_div.val(), second_photo_div.val()];

      if ( ( $("#account_live_photo_1").val() ) && ( $("#account_live_photo_2").val() ) ) {
      	$('.camera-cont').remove();
      	$('.camera-submit').show();
        alertNotify("2");
        $("#submit_photos").addClass("btn-default").prop("disabled", false);
      }
      else {
        alertNotify("1");
      }
...
function alertNotify(count) {
	$.gritter.add({
		title: 'Shoot was taken!',
		text: 'You have created ' + count + ' out of 2 photos.',
